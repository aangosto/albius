...
- `TODO[delete-on-empty-fields]` — vaciar campos opcionales preexistentes (`nombreComercial`, `logoUrl`) en el form de edición no envía el cambio al backend porque el callable no soporta delete-on-empty. Limitación: el operador no puede "quitar" un nombre comercial una vez asignado. Soluciones cuando surja necesidad: (a) callable acepta `FieldValue.delete()` sentinel; (b) frontend traduce `""` a sentinel de borrado. Origen: Bloque 10.
